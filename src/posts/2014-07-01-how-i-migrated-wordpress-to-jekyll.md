...
date: 2014-07-01
image: wordpress-jekyll-migration.jpg
permalink: /work/how-i-migrated-from-wordpress-to-jekyll/
---

**Update:** Tweets no longer exist so links removed

Last weekend I migrated my personal blog from [Wordpress][1] to [Jekyll][2]. I tweeted as I went to keep some sort of record of what I\'d done and the decisions I\'d made. I also hoped that anyone else wanting to do the same thing would find it helpful.
...
>
> — Ben Chadfield (@benchadfield) June 28, 2014


That last part about hosting on Github isn\'t true. I hosted it on the [CS Workflow][5] server.
...
Jekyll uses a template, processing rules, and content to generate a static site. Each page of the site has its .html file. The HTML file, along with the stylesheets, scripts, and images, can be uploaded to any web server. There are tons of [ways to host static sites][6] and almost all of them are free.
...
Off to google to find a good guide. I didn\'t want to spend a lot of time or energy on this migration, so a good tutorial saves both. I recommend the [how-to guide on girliemac.com][11]. This is what I followed. I\'ll assume your also using that as a reference and only chime in if I have something to add.
...
I can\'t remember what I searched for, but I found [Poole][21], a Jekyll start kit built by Mark Otto of Bootstrap fame. Poole already looks pretty good, but has two themes. I used the [Hyde][22] theme, which I made a few changes to.
...
* Added a background cover image for the left column. I got the image from [this awesome photo resource][23].
...
>
> — Ben Chadfield (@benchadfield) June 29, 2014

I needed to host the site myself because there are a few restrictions to hosting on Github. The main one that stopped me going with that option is not being able to use plugins. I use two plugins. The first is to [figure out how old][26] my daughter is. This is displayed in my bio in the left column. Jekyll uses [Liquid templates][27], but doesn\'t come with a timeago filter as standard. The second plugin is [jekyll-assets][28], which compiles the different stylesheets into a single, minified, gzipped, and cache-busting css file. This makes loading for the first time even faster.

To start, I created a new Github repository called [chadfield-blog][29]. This gives me cloud-hosted source control using git. Github also makes deploying changes really easy. To get the site on to the server I sshed in and ran `git clone https://github.com/bchadfield/chadfield-blog.git` from the directory I wanted to serve the site from. To get any future updates I run `git pull`.
...
I automate steps 5, 6, and 7 using a rakefile. I use [CS Workflow][5] to write, get reviews, and export meta-data to Markdown.
